...
static ER create_shravya_tasks(void)
{
    ...
    /* ✅ Task 2: Signal Processing Task */
       ...
       ctsk.itskpri = 3;
       ctsk.stksz = 8192;
       printf("SHRAVYA: About to create Task 2 (Signal Processing)\n");
       task_id = tk_cre_tsk(&ctsk);
       if (task_id <= 0) {
           printf("SHRAVYA: ❌ Task 2 creation FAILED: %d\n", task_id);
           return E_SYS;
       }
       printf("SHRAVYA: Task 2 created (priority %d)\n", ctsk.itskpri);

       ercd = tk_sta_tsk(task_id, 0);
       if (ercd != E_OK) {
           printf("SHRAVYA: ❌ Task 2 start FAILED: %d\n", ercd);
           return ercd;
       }
       printf("SHRAVYA: Task 2 started\n");
