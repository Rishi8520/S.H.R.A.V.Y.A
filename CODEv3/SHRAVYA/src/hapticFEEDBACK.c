...
/**
 * ✅ ADD: Session Management Functions - CRITICAL FOR TRON CONTEST
 */

/* Session Pattern Definitions */
typedef enum {
    SESSION_START_PATTERN = 0,
    SESSION_END_PATTERN,
    DROWSINESS_ALERT_PATTERN,
    SYSTEM_READY_PATTERN
} session_pattern_type_t;

/**
 * @brief Trigger session start notification
 */
fsp_err_t trigger_session_start(void)
{
    if (!haptic_initialized) return FSP_ERR_NOT_INITIALIZED;

    // Gentle double pulse - left motor only
    haptic_pattern_t session_start = {
        .step_count = 4,
        .repeat_count = 1,
        .bilateral = false,
        .pattern_name = "SessionStart"
    };

    // Pattern: Short pulse, pause, longer pulse, pause
    session_start.steps[0] = (haptic_step_t){50, 0, 200, true, true};   // Left quick pulse
    session_start.steps[1] = (haptic_step_t){0, 0, 150, false, false};  // Pause
    session_start.steps[2] = (haptic_step_t){60, 0, 400, true, true};   // Left longer pulse
    session_start.steps[3] = (haptic_step_t){0, 0, 300, false, false};  // End pause

    haptic_state.current_pattern = session_start;
    haptic_state.current_step = 0;
    haptic_state.step_timer_ms = 0;
    haptic_state.repeat_counter = 0;
    haptic_state.pattern_active = true;
    haptic_state.active_intervention = COGNITIVE_STATE_CALM;

    return FSP_SUCCESS;
}

/**
 * @brief Trigger session end notification
 */
fsp_err_t trigger_session_end(void)
{
    if (!haptic_initialized) return FSP_ERR_NOT_INITIALIZED;

    // Triple pulse - right motor only
    haptic_pattern_t session_end = {
        .step_count = 6,
        .repeat_count = 1,
        .bilateral = false,
        .pattern_name = "SessionEnd"
    };

    // Pattern: Three distinct pulses on right motor
    session_end.steps[0] = (haptic_step_t){0, 45, 150, false, false};   // Right pulse 1
    session_end.steps[1] = (haptic_step_t){0, 0, 100, false, false};    // Pause
    session_end.steps[2] = (haptic_step_t){0, 50, 150, false, false};   // Right pulse 2
    session_end.steps[3] = (haptic_step_t){0, 0, 100, false, false};    // Pause
    session_end.steps[4] = (haptic_step_t){0, 60, 200, false, true};    // Right pulse 3 with fade
    session_end.steps[5] = (haptic_step_t){0, 0, 400, false, false};    // End pause

    haptic_state.current_pattern = session_end;
    haptic_state.current_step = 0;
    haptic_state.step_timer_ms = 0;
    haptic_state.repeat_counter = 0;
    haptic_state.pattern_active = true;
...
/**
 * @brief Trigger drowsiness/dozing alert - HIGH PRIORITY
 */
fsp_err_t trigger_drowsiness_alert(void)
{
    if (!haptic_initialized) return FSP_ERR_NOT_INITIALIZED;

    // Strong bilateral alert pattern - WAKE UP USER
    haptic_pattern_t drowsiness_alert = {
        .step_count = 8,
        .repeat_count = 3,  // Repeat 3 times for strong effect
        .bilateral = true,
        .pattern_name = "DrowsinessAlert"
    };

    // Pattern: Escalating intensity to grab attention
    drowsiness_alert.steps[0] = (haptic_step_t){70, 70, 100, false, false};  // Strong burst
    drowsiness_alert.steps[1] = (haptic_step_t){0, 0, 50, false, false};     // Short pause
    drowsiness_alert.steps[2] = (haptic_step_t){85, 85, 120, false, false};  // Stronger burst
    drowsiness_alert.steps[3] = (haptic_step_t){0, 0, 50, false, false};     // Short pause
    drowsiness_alert.steps[4] = (haptic_step_t){95, 95, 150, false, false};  // Very strong
    drowsiness_alert.steps[5] = (haptic_step_t){0, 0, 100, false, false};    // Pause
    drowsiness_alert.steps[6] = (haptic_step_t){80, 80, 300, true, true};    // Long fading alert
    drowsiness_alert.steps[7] = (haptic_step_t){0, 0, 500, false, false};    // Recovery pause

    // Force stop any current pattern
    if (haptic_state.pattern_active) {
        haptic_state.pattern_active = false;
        set_motor_intensity(0, 0);
    }

    haptic_state.current_pattern = drowsiness_alert;
    haptic_state.current_step = 0;
    haptic_state.step_timer_ms = 0;
    haptic_state.repeat_counter = 0;
    haptic_state.pattern_active = true;
    haptic_state.active_intervention = COGNITIVE_STATE_FATIGUE;
...
/**
 * @brief System ready notification (for testing/debug)
 */
fsp_err_t trigger_system_ready(void)
{
    if (!haptic_initialized) return FSP_ERR_NOT_INITIALIZED;

    // Single strong bilateral pulse to indicate system is ready
    haptic_pattern_t system_ready = {
        .step_count = 2,
        .repeat_count = 1,
        .bilateral = true,
        .pattern_name = "SystemReady"
    };

    system_ready.steps[0] = (haptic_step_t){80, 80, 300, true, true};  // Strong confirmation
    system_ready.steps[1] = (haptic_step_t){0, 0, 200, false, false};  // End

    haptic_state.current_pattern = system_ready;
    haptic_state.current_step = 0;
    haptic_state.step_timer_ms = 0;
    haptic_state.repeat_counter = 0;
    haptic_state.pattern_active = true;
...
/**
 * @brief Emergency stop all haptic activity
 */
fsp_err_t haptic_emergency_stop(void)
{
    haptic_state.pattern_active = false;
    haptic_state.pattern_paused = true;
    set_motor_intensity(0, 0);
