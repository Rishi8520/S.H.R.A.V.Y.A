...
/* ✅ WORKING DEBUG OUTPUT - Uses semi-hosting printf */
...
ER tk_ini_ker(void)
{
    ...
    debug_output("μT-Kernel 3.0 initialized successfully");
    return E_OK;
}
...
ER tk_ext_ker(void)
{
    ...
    debug_output("Starting μT-Kernel 3.0 scheduler...");
...
/**
 * @brief Signal Semaphore from ISR - FIXED VERSION
 */
ER tk_isig_sem(ID semid, INT cnt)
{
    (void)cnt;  // For now, ignore count and just signal once
    ...
    int semidx = semid - 1;
    if (!semaphore_table[semidx].active) {
        return E_NOEXS;
    }

    /* Signal semaphore - ISR version */
    if (semaphore_table[semidx].count < semaphore_table[semidx].max_count) {  // ✅ CORRECT FIELD NAME
        semaphore_table[semidx].count++;
        debug_output("ISR Semaphore signaled");
        return E_OK;
    }

    debug_output("ISR Semaphore full");
    return E_LIMIT;
}
...
ID tk_cre_sem(T_CSEM *pk_csem)
{
    ...
    for (int i = 0; i < MAX_SEMAPHORES; i++) {
        if (!semaphore_table[i].active) {
            ...
            printf("SHRAVYA: Semaphore %d created\r\n", i + 1);
            return i + 1;
        }
    }
...
ER tk_wai_sem(ID semid)
{
    ...
    /* FIXED: Actually wait for semaphore instead of returning immediately */
    volatile int wait_loops = 0;
    const int MAX_WAIT_LOOPS = 10000; /* For testing - smaller timeout */
    ...
    while (wait_loops < MAX_WAIT_LOOPS) {
        ...
        for (volatile int i = 0; i < 100; i++); /* Brief busy wait */
        wait_loops++;
    }

    /* Return timeout if no semaphore signal received */
    return E_TMOUT;
}
...
ID tk_cre_mtx(T_CMTX *pk_cmtx)
{
    ...
    for (int i = 0; i < MAX_MUTEXES; i++) {
        if (!mutex_table[i].active) {
            ...
            printf("SHRAVYA: Mutex %d created\r\n", i + 1);
            return i + 1;
        }
    }
...
ER tk_sus_tsk(ID tskid)
{
    ...
    task_table[task_idx].running = false;
    printf("SHRAVYA: Task %d suspended\r\n", (int)tskid);
    return E_OK;
}
...
ER tk_rsm_tsk(ID tskid)
{
    ...
    task_table[task_idx].running = true;
    printf("SHRAVYA: Task %d resumed\r\n", (int)tskid);
    return E_OK;
}

/**
 * @brief Release Wait State (simplified version)
 */
ER tk_rel_wai(void)
{
    printf("SHRAVYA: Wait state released\r\n");
    return E_OK;
}
...
ID tk_cre_flg(T_CFLG *pk_cflg)
{
    ...
    for (int i = 0; i < MAX_EVENTFLAGS; i++) {
        if (!eventflag_table[i].active) {
            ...
            printf("SHRAVYA: Event flag %d created\r\n", i + 1);
            return i + 1;
        }
    }
...
{
    printf("SHRAVYA: Power mode set to %d\r\n", powmode);
    return E_OK;
}
