...
#define PROCESSING_WINDOW_SIZE      256     // 512ms at 500Hz (power of 2)
...
void task_preprocessing_entry(INT stacd, void *exinf)
{
    ...
    eeg_raw_sample_t raw_samples[10]; // Process 10 samples at once (from 500Hz to 50Hz)
    ...
    while(1)
    {
        ...
        if (eeg_get_samples(raw_samples, 10, &samples_read) == FSP_SUCCESS && samples_read > 0)
        {
